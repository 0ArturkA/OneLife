    ...
    if (inPlayer->numContained != 0)
    {
             ...
             c++)
        {
            ...
            if (numSub > 0)
            {
                ...
                timeSec_t *subDecays =
                    inPlayer->subContainedEtaDecays[c].getElementArray();
        ...
        for (int c = 0; c < inPlayer->numContained; c++)
        {
            ...
            if (hasSub)
            {
                     ...
                     s < inPlayer->subContainedIDs[c].size(); s++)
                {
                    ...
                    ObjectRecord *subO =
                        getObject(inPlayer->subContainedIDs[c].getElementDirect(s));
    ...
    for (int c = 0; c < NUM_CLOTHING_PIECES; c++)
    {
        ...
        if (cO != NULL)
        {
                 ...
                 s < inPlayer->clothingContained[c].size(); s++)
            {
                ...
                ObjectRecord *sO =
                    getObject(inPlayer->clothingContained[c].getElementDirect(s));
                             ...
                             NULL)
{
    ...
    for (int p = 0; p < inPlayer->pathLength; p++)
    {
        ...
        char *stepString = autoSprintf(" %d %d",
                                       inPlayer->pathToDest[p].x - inPlayer->xs,
                                       inPlayer->pathToDest[p].y - inPlayer->ys);
    ...
    if (inNewObject->blocksWalking)
    {
        ...
        for (int j = 0; j < numLive; j++)
        {
                ...
                otherPlayer->yd != otherPlayer->ys)
            {
                ...
                if (distance(cPos, dropSpot) <= 2 * pathDeltaMax)
                {
                        ...
                        blockedStep > 0)
                    {
                        ...
                        otherPlayer->pathLength = blockedStep;
                        otherPlayer->pathTruncated = true;
                        ...
                        otherPlayer->xd = otherPlayer->pathToDest[blockedStep - 1].x;
                        otherPlayer->yd = otherPlayer->pathToDest[blockedStep - 1].y;
                    }
                    else if (blocked)
                    {
                        // cutting off path
    ...
    UpdateRecord *inRecord, GridPos inRelativeToPos)
{
    ...
    if (inRecord->posUsed)
    {
        return autoSprintf(inRecord->formatString,
                           inRecord->absoluteActionTarget.x - inRelativeToPos.x,
                           inRecord->absoluteActionTarget.y - inRelativeToPos.y,
                                       ...
                                       int inC)
{
            ...
            inPlayer->holdingID))
    {
        ...
        float containSize =
            getObject(inPlayer->holdingID)->containSize;

        if (oldNum < cObj->numSlots &&
            containSize <= slotSize)
        {
            // room
            inPlayer->clothingContained[inC].push_back(
                inPlayer->holdingID);
            ...
            inPlayer->clothingContainedEtaDecays[inC].push_back(inPlayer->holdingEtaDecay);
                                              ...
                                              int inI = -1)
{
        ...
        slotToRemove >= 0)
    {
        ...
        toRemoveID =
            inPlayer->clothingContained[inC].getElementDirect(slotToRemove);
    }
            ...
            computeAge(inPlayer))
    {
        ...
        inPlayer->holdingEtaDecay =
            inPlayer->clothingContainedEtaDecays[inC].getElementDirect(slotToRemove);
        ...
        if (inPlayer->holdingEtaDecay != 0)
        {
            ...
            timeSec_t offset =
                inPlayer->holdingEtaDecay - curTime;
            offset = offset * stretch;
            inPlayer->holdingEtaDecay =
                curTime + offset;
        }

        inPlayer->clothingContained[inC].deleteElement(slotToRemove);
        inPlayer->clothingContainedEtaDecays[inC].deleteElement(slotToRemove);
        ...
        if (!apocalypseStarted)
        {
                ...
                apocalypseRequest == NULL && reflectorURL != NULL)
            {
                ...
                if (reflectorSharedSecret != NULL)
                {
                    ...
                    {
                        name =
                            players.getElement(closestPlayerIndex)->name;
                    }
                      ...
                      SimpleVector<int> *inPlayerIndicesToSendHealingAbout)
{
    ...
    if (inPlayer->murderPerpEmail !=
        NULL)
    {
        delete[] inPlayer->murderPerpEmail;
    ...
    inPlayerIndicesToSendHealingAbout->push_back(
        getLiveObjectIndex(
            inPlayer->id));
}
    ...
    while (!quit)
    {
        ...
        for (int i = 0; i < numLive; i++)
        {
            ...
            for (int c = 0; c < NUM_CLOTHING_PIECES; c++)
            {
                ...
                {
                    timeSec_t decay =
                        nextPlayer->clothingContainedEtaDecays[c].getElementDirect(cc);
        ...
        for (int i = 0; i < newConnections.size(); i++)
        {
            ...
            else if (nextConnection->ticketServerRequest != NULL)
            {
                ...
                if (currentTime - nextConnection->ticketServerRequestStartTime < 8)
                {
                    ...
                    if (strstr(message, "LOGIN") != NULL)
                    {
                            ...
                            tokens->size() == 7)
                        {
                                ...
                                !nextConnection->error)
                            {
                                ...
                                nextConnection->ticketServerRequestStartTime = currentTime;
                ...
                !wasRecentlyDeadly(curPos))
            {
                    ...
                    curOverObj->permanent && curOverObj->deadlyDistance > 0)
                {
                    ...
                    if (!nextPlayer->dying || wasSick)
                    {
                        ...
                        playerIndicesToSendDyingAbout.push_back(
                            getLiveObjectIndex(
                                nextPlayer->id));
                    }
                    else
                                ...
                                if (babyBonesO != NULL)
                                {
                                         ...
                                         c++)
                                    {
                                        ...
                                        if (cObj != NULL)
                                        {
                                            ...
                                            if (nextPlayer->clothingContained[c].size() > 0)
                                            {
                                                ...
                                                parent->containedIDs =
                                                    nextPlayer->clothingContained[c].getElementArray();
                                                parent->containedEtaDecays =
                                                    nextPlayer->clothingContainedEtaDecays
                                                        [c]
                                                            .getElementArray();

                                                parent->subContainedIDs = new SimpleVector<int>[parent->numContained];
                                                parent->subContainedEtaDecays = new SimpleVector<timeSec_t>[parent->numContained];
                                            }
                         ...
                         m.type == EMOT)
                {
                        ...
                        nextPlayer->heldByOther)
                    {
                        ...
                        // in-arms, until they JUMP out
                             ...
                             getObject(nextPlayer->holdingID)->speedMult == 0)
                    {
                        ...
                        */
                        ...
                        else
                        {
                            ...
                            */
                            ...
                            if (!startFound &&
                                !isGridAdjacentDiag(
                                    unfilteredPath.getElementDirect(startIndex).x,
                            ...
                            else
                            {
                                ...
                                nextPlayer->xd =
                                    nextPlayer->pathToDest[nextPlayer->pathLength - 1].x;
                                nextPlayer->yd =
                                    nextPlayer->pathToDest[nextPlayer->pathLength - 1].y;
                                 ...
                                 minSayGapInSeconds)
                    {
                        ...
                        for (int c = 0; c < len; c++)
                        {
                            if (!allowedSayCharMap[(int)(m.saidText[c])])
                            {
                            ...
                            if (heldObj->deadlyDistance > 0)
                            {
                                                       ...
                                                       targetPos))
                                {
                                    ...
                                    if (hitPlayer != NULL)
                                    {
                                        ...
                                        if (hitPlayer->murderPerpEmail != NULL)
                                        {
                                            delete[] hitPlayer->murderPerpEmail;
                                        }
                                    ...
                                    if (r != NULL)
                                    {
                                            ...
                                            r->newTarget != 0)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    // no further decay
                                                    hitPlayer->embeddedWeaponEtaDecay = 0;
                                                }
                                            }
                                        }
                            ...
                            else if (target != 0)
                            {
                                ...
                                else if (nextPlayer->holdingID > 0)
                                {
                                        ...
                                        r->newTarget > 0)
                                    {
                                        ...
                                        if (targetObj->numSlots == 0 && newTargetObj->numSlots >= heldObj->numSlots &&
                                    ...
                                    r->newTarget != target)
                                {
                                    ...
                                    if (getMapFloor(m.x, m.y) != 0)
                                    {
                                        ...
                                        // to exist on floor
                                        if (strstr(getObject(r->newTarget)->description,
                                                   "groundOnly") != NULL)
                                        {
                                            r = NULL;
                                        }
                                ...
                                if (r != NULL && containmentTransfer)
                                {
                                    ...
                                    nextPlayer->heldGraveOriginY = m.y;
                                         ...
                                         (r->newTarget == 0 ||
                                          !(getObject(r->newTarget)->blocksWalking &&
                                            getObject(r->newTarget)->floorHugging) ||
                                                                  ...
                                                                  m.y)))
                                {
                                    ...
                                    if (r->newTarget > 0 && getObject(r->newTarget)->floor)
                                    {
                            ...
                            else if (nextPlayer->holdingID > 0)
                            {
                                ...
                                if (floorID > 0)
                                {
                                             ...
                                             computeAge(nextPlayer)))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            char canPlace = true;
                                            if (getObject(resultID)->blocksWalking &&
                                ...
                                if (!usedOnFloor && obj->foodValue == 0)
                                {
                                             ...
                                             computeAge(nextPlayer)))
                                    {
                                        ...
                                        if (getObject(r->newTarget)->blocksWalking &&
                                            !isMapSpotEmpty(m.x, m.y))
                                        {
                                    ...
                                    if (canPlace)
                                    {
                                        ...
                                        if (nextPlayer->numContained > 0 &&
                                            r->newActor == 0 &&
                                            r->newTarget > 0 &&
                                            getObject(r->newTarget)->numSlots >= nextPlayer->numContained &&
                                            getObject(r->newTarget)->slotSize >= obj->slotSize)
                                        {
                                    ...
                                    if (isFertileAge(nextPlayer))
                                    {
                                        ...
                                        if (nextPlayer->yummyBonusStore > 0)
                                        {
                                                ...
                                                0)
                                            {
                                                ...
                                                nurseCost = -nextPlayer->yummyBonusStore;
                    ...
                    else if (m.type == SELF || m.type == UBABY)
                    {
                        ...
                        if (nextPlayer->holdingID > 0)
                        {
                            ...
                            if (obj->foodValue > 0)
                            {
                                ...
                                if (strstr(obj->description, "remapStart") != NULL)
                                {
                                ...
                                if (healTrans != NULL)
                                {
                                    ...
                                    if (healerWillHold > 0)
                                    {
                                        if (computeAge(nextPlayer) <
                                            getObject(healerWillHold)->minPickupAge)
                                        {
                                            oldEnough = false;
                                        }
                                    }
                            ...
                            else if (nextPlayer->holdingID > 0)
                            {
                                              ...
                                              babyAge))
                                {
                                    ...
                                    case 'p':
                                        ...
                                        break;
                                    case 's':
                                        if (targetPlayer->clothing.backShoe == NULL)
                                        {
                                    ...
                                    if (clothingSlot != NULL)
                                    {
                                        ...
                                        oldNumContained =
                                            targetPlayer->clothingContained[ind].size();

                                        if (oldNumContained > 0)
                                        {
                                            oldContainedIDs =
                                                targetPlayer->clothingContained[ind].getElementArray();
                                            oldContainedETADecays =
                                                targetPlayer->clothingContainedEtaDecays[ind].getElementArray();
                                        }
                                        ...
                                        *clothingSlot = obj;
                                        ...
                                        targetPlayer->clothingContained[ind].deleteAll();
                                        targetPlayer->clothingContainedEtaDecays[ind].deleteAll();
                                        ...
                                        if (nextPlayer->numContained > 0)
                                        {
                                            ...
                                            targetPlayer->clothingContainedEtaDecays[ind]
                                                .appendArray(
                                                    nextPlayer->containedEtaDecays,
                                                    nextPlayer->numContained);
                                        ...
                                        if (oldC != NULL)
                                        {
                                            ...
                                            nextPlayer->holdingEtaDecay = oldCEtaDecay;
                                            ...
                                            nextPlayer->subContainedIDs = new SimpleVector<int>[nextPlayer->numContained];
                                            nextPlayer->subContainedEtaDecays = new SimpleVector<timeSec_t>[nextPlayer->numContained];
                                        }
                                    }
                                }
                            }
                            else
                            {
                                    ...
                                    targetPlayer->clothing.frontShoe != NULL)
                                {
                                    ...
                                    clothingSlotIndex = 2;
                                         ...
                                         targetPlayer->clothing.backShoe != NULL)
                                {
                                    clothingSlot =
                                        &(targetPlayer->clothing.backShoe);
                                    clothingSlotIndex = 3;
                                         ...
                                         targetPlayer->clothing.hat != NULL)
                                {
                                    ...
                                    clothingSlotIndex = 0;
                                         ...
                                         targetPlayer->clothing.tunic != NULL)
                                {
                                    clothingSlot =
                                        &(targetPlayer->clothing.tunic);
                                    clothingSlotIndex = 1;
                                         ...
                                         targetPlayer->clothing.bottom != NULL)
                                {
                                    clothingSlot =
                                        &(targetPlayer->clothing.bottom);
                                    clothingSlotIndex = 4;
                                         ...
                                         targetPlayer->clothing.backpack != NULL)
                                {
                                ...
                                if (clothingSlot != NULL)
                                {
                                    ...
                                    *clothingSlot = NULL;
                                    ...
                                    nextPlayer->subContainedIDs =
                                        new SimpleVector<int>[nextPlayer->numContained];

                                    nextPlayer->subContainedEtaDecays =
                                        new SimpleVector<timeSec_t>[nextPlayer->numContained];
                            ...
                            if (nextPlayer->holdingID != 0)
                            {
                                ...
                                if (nextPlayer->holdingID < 0)
                                {
                                    ...
                                    if (target == 0 // nothing here
                                        ||
                                        !getObject(target)->blocksWalking)
                                    {
                                        handleDrop(
                                            m.x, m.y, nextPlayer,
                                            &playerIndicesToSendUpdatesAbout);
                                         ...
                                         nextPlayer->holdingID > 0)
                                {
                                    ...
                                    if (nextPlayer->holdingID == 0)
                                    {
                                        ...
                                        // holding before
                                        for (int s = 0;
                                             s < nextPlayer->clothingContained[m.c].size() - 1;
                                             s++)
                                        {

                                            if (nextPlayer->clothingContained[m.c].getElementDirect(s) !=
                                                oldHeld)
                                            {
                                ...
                                else if (nextPlayer->holdingID > 0)
                                {
                                    ...
                                    ObjectRecord *droppedObj = getObject(
                                        nextPlayer->holdingID);
                                                     ...
                                                     computeAge(nextPlayer))
                                        {
                                                ...
                                                nextPlayer->holdingID)
                                            {
                                                ...
                                                if (oldHeld == target)
                                                {
                                                    // try using held
                                                    // on target
                                                    TransRecord *sameTrans = getPTrans(
                                                        oldHeld, target);
                                                    if (sameTrans != NULL &&
                                                        sameTrans->newActor ==
                                                            0)
                            ...
                            if (target > 0)
                            {
                                ...
                                else if (targetObj->permanent)
                                {
                                    ...
                                    if (handTrans != NULL &&
                                        ...
                                        getObject(handTrans->newTarget)->numSlots == targetObj->numSlots &&
                                        handTrans->newActor > 0 &&
                                        getObject(handTrans->newActor)->minPickupAge <=
                                            computeAge(nextPlayer))
                                    {
                ...
                if (deathID != 0)
                {
                    ...
                    while (oldObject != 0 && n <= 4)
                    {
                        ...
                        if (!isGrave(oldObject))
                        {
                            ...
                            if (r->numSlots == 0 && !r->permanent && !r->rideable)
                            {
                    ...
                    if (deathID > 0)
                    {
                             ...
                             c++)
                        {
                                 ...
                                 cc++)
                            {
                                ...
                                if (nextPlayer->clothingContainedEtaDecays[c].getElementDirect(cc) != 0 &&
                                    oldStretch != newStretch)
                                {
                                    ...
                                    *(nextPlayer->clothingContainedEtaDecays[c].getElement(cc)) =
                                        curTime + offset;
                                }

                                addContained(
                                    dropPos.x, dropPos.y,
                                    nextPlayer->clothingContained[c].getElementDirect(cc),
                                    nextPlayer->clothingContainedEtaDecays[c].getElementDirect(cc));
                                roomLeft--;
                            }
                        }
            ...
            else if (!nextPlayer->error)
            {
                    ...
                    for (int c = 0; c < nextPlayer->numContained; c++)
                    {
                        ...
                        if (newDecay != 0 && newDecay < curTime)
                        {
                            ...
                            if (t != NULL)
                            {
                                ...
                                {
                                    float stretch =
                                        getObject(nextPlayer->holdingID)->slotTimeStretch;
                        ...
                        for (int s = 0; s < subCont.size(); s++)
                        {
                            ...
                            if (newSubDecay != 0 && newSubDecay < curTime)
                            {
                                ...
                                if (t != NULL)
                                {
                                    ...
                                    {
                                        float subStretch =
                                            getObject(newID)->slotTimeStretch;
                ...
                for (int c = 0; c < NUM_CLOTHING_PIECES; c++)
                {
                            ...
                            curTime)
                    {
                        ...
                        if (t != NULL)
                        {
                            ...
                            if (newSlots < oldSlots)
                            {
                                ...
                                for (int s = newSlots; s < oldSlots; s++)
                                {
                                    ...
                                    if (found)
                                    {
                                        setMapObject(
                                            spot.x, spot.y,
                                            nextPlayer->clothingContained[c].getElementDirect(s));

                                        timeSec_t eta =
                                            nextPlayer->clothingContainedEtaDecays[c].getElementDirect(s);
                                ...
                                nextPlayer->clothingContained[c].shrink(newSlots);

                                nextPlayer->clothingContainedEtaDecays[c].shrink(newSlots);
                            }
                            ...
                            if (oldStretch != newStretch)
                            {
                                ...
                                for (int cc = 0;
                                     cc < nextPlayer->clothingContainedEtaDecays[c].size();
                                     cc++)
                                {
                        ...
                        nextPlayer->clothingContainedEtaDecays[c].size() > 0)
                    {
                        ...
                        for (int cc = 0;
                             cc <
                             nextPlayer->clothingContainedEtaDecays[c].size();
                             cc++)
                        {

                            int oldID = nextPlayer->clothingContained[c].getElementDirect(cc);
                            int newID = oldID;
                            ...
                            if (adultO != NULL)
                            {
                                    ...
                                    otherPlayer->id != nextPlayer->id)
                                {
                                    ...
                                    if (d <= getMaxChunkDimension() / 2)
                                    {
                                        ...
                                        chunkPlayerUpdates.appendElementString(updateLine);
                                        delete[] updateLine;
                                    }
                                }
                            }
                    ...
                    if (middleDistancePlayerIDs.size() > 0)
                    {
                        ...
                        if (middleDistancePlayerIDs.size() > 0)
                        {
                                 ...
                                 i < middleDistancePlayerIDs.size(); i++)
                            {
                                ...
                                sprintf(
                                    buffer, "%d\n",
                                    middleDistancePlayerIDs.getElementDirect(i));
